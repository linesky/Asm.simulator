...
class BareboneBuilder:
    ...
    def build_kernel(self):
        ...
        fff=f'as   ./file/boot.s -o /tmp/boot.o'
        
        self.execute_command(fff,True)
        
        fff=f'nasm -f elf32 "$1" -o /tmp/hello.o'.replace("$1",filename)
        
        self.execute_command(fff,True)
        fff=f'ld  -T ./file/link.ld -nostandlib /tmp/boot.o /tmp/hello.o  -o /tmp/kernel.bin'
        
        self.execute_command(fff,True)
        self.execute_command("cp -f /tmp/kernel.bin /tmp/hello.c32",True)
